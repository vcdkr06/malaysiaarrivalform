...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-[85vh] flex items-center overflow-hidden bg-white">
      {/* ——— IMAGE CONTAINER: Pushed further left for more visibility ——— */}
      <div className="absolute top-0 right-0 w-full lg:w-[75%] h-full z-0">
        ...
        />
        {/* Extreme White Fade: Moved far to the left to reveal more photo */}
        <div className="absolute inset-0 bg-gradient-to-b from-white via-white/40 to-white/80 lg:bg-gradient-to-r lg:from-white lg:via-white lg:via-[10%] lg:to-transparent lg:to-[60%]" />
      </div>

      <div className="container mx-auto px-6 relative z-10 max-w-6xl">
        <div className="grid lg:grid-cols-[1fr_0.75fr] gap-6 lg:gap-16 items-center">
          {/* ——— LEFT COLUMN: Focused & Clean ——— */}
          <div className="flex flex-col space-y-4 lg:space-y-6 pt-4 lg:pt-0">
            <div className="flex items-center gap-2">
              {/* Yellow Mandatory Badge */}
              <span className="px-3 py-1.5 rounded-md bg-yellow-400 text-slate-900 text-[10px] font-black uppercase tracking-widest shadow-md flex items-center gap-2">
            ...
            <div className="space-y-1 lg:space-y-2">
              ...
              <p className="text-sm md:text-base text-slate-500 max-w-sm leading-relaxed font-medium">
                Official digital registration for the mandatory Malaysia Arrival Card (MDAC) for all international
                visitors.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <Button
                asChild
                size="lg"
                className="w-full sm:w-auto h-14 px-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg shadow-xl shadow-blue-200 border-0 transition-transform active:scale-95"
              >
          ...
          <div className="relative pb-8 lg:pb-0">
            {/* The Glass Card - Backdrop increased for contrast against the clearer photo */}
            <div className="relative bg-white/90 backdrop-blur-xl border border-white rounded-[2rem] p-6 lg:p-8 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)]">
              ...
              <div className="grid grid-cols-2 gap-4">
                {steps.map((step, i) => (
                  <div
                    key={i}
                    className="flex flex-col items-center text-center p-4 rounded-2xl bg-white border border-slate-50 shadow-sm transition-all hover:border-blue-100 group"
                  >
                    <div
                      className={`w-11 h-11 rounded-xl ${step.bg} ${step.color} flex items-center justify-center mb-2 shadow-sm border border-white transition-transform group-hover:scale-105`}
                    >
                      {step.icon}
                    </div>
      ...
      <div className="hidden lg:block absolute bottom-6 left-12 opacity-30">
        <p className="text-[9px] text-slate-600 tracking-[0.2em] font-bold uppercase">
          Independent Concierge Agency • Professional Review • No Government Affiliation
        </p>
      </div>
    </section>
  );
